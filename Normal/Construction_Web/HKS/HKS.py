...
# 'if-none-match': "9d8a8884f17498bf3e4f61ea4de715b3-ssl-df",
...
def getInfos(url):
    ...
    if len(featureItem) != 0:
        ...
        feature = '\n'.join([item.split(':')[-1].strip('"').strip() for item in re.findall(feature_reg, html)[0].split('},{')])
    else:
        feature = None
     
    if len(awardItem) != 0:
        award = '\n'.join([item.split(':')[-1].strip('"').strip() for item in re.findall(awards_reg, html)[0].split('},{')])
    else:
        award = None
        
    return feature, award

# aurl = 'https://www.hksinc.com/path---what-we-do-case-studies-the-farm-at-crossroad-commons-f855b9a84dd82f6880a1.js'
# content = getInfos(aurl)
    ...
    if each['_source']['post_type'] != None and 'case_study' in each['_source']['post_type']:
        if each['_source']['locations'] != None and 'Dallas' in each['_source']['locations']:
            if each['_source']['services'] != None and 'Architecture' in each['_source']['services']:
                ...
                service = '/'.join([single.strip() for single in each['_source'].get('services',None)])
                locations = '/'.join([single.strip() for single in each['_source'].get('locations',None)])
                ...
                if project_types != None:
                    project_types = '/'.join([single.strip() for single in project_types])

                body = each['_source'].get('body',None)
                if body != None:
                    body = '\n'.join([list(single.values())[0].replace('<p>', '').replace('</p>','').replace("’s", "'s").strip() for single in body])

                inner = title.replace(' ','-')
                inner = ''.join([single if (single.isalnum() or single == '-') else '' for single in inner])
                ...
                print(title)
                zeta.append([title, service, locations, body, features, awards])
...
zetaToSave.to_csv('../HKS.csv', index = False)
